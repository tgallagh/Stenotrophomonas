...
DEST=/bio/tgallagh/Stenotrophomonas/data/processed/CFdata/Whiteley/
bowtie2 --very-fast -I 0 --no-unal -q -x $ref/Sm_index -U $input  -S $DEST$input\.sam
samtools view -bS $DEST$input\.sam | samtools sort > $DEST$input\.sorted.bami

DEST2=/bio/tgallagh/Stenotrophomonas/data/processed/CFdata/Whiteley/
bowtie2 --very-fast -I 0 --no-unal -q -x $ref2/Smindex -U $input  -S $DEST2$input\.sam
samtools view -bS $DEST2$input\.fastafile.sam | samtools sort > $DEST2$input\.fastafile.sorted.bami
